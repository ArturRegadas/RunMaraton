...
def run_with_limits_linux(
    executable, input_file, output_file,username,
    timelimit=1.0,
    number_of_repetitions=10,
    memory_limit_MB=128,
    output_limit_KB=64,
):
    user_output_path = os.path.join(os.getcwd(), "submissions", "outputs", username)
    ...
    try:
        for _ in range(number_of_repetitions):
            with open(input_file, 'rb') as f_in:
                ...
                with open(user_output_path+".txt", 'wb') as f_out:
                    f_out.write(stdout)

        with open(user_output_path+".txt", 'r', encoding='utf-8', errors='ignore') as uo, \
            ...
            with open(os.path.join(os.getcwd(), "submissions", "error",username+".txt" ), "w") as file:
